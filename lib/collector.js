; (function () {
    'use strict'
    window.ViolentRipper = window.ViolentRipper || {}
    ...
    ViolentRipper.collector = {
        ...
        collectJs() {
            ...
            if (window.performance) {
                performance.getEntriesByType('resource').forEach(r => {
                    if (r.initiatorType === 'script' || r.name.match(/\.js(\?|$)/)) {
        ...
        collectCss() {
            ...
            if (window.performance) {
                performance.getEntriesByType('resource').forEach(r => {
                    if (r.initiatorType === 'css' || r.name.match(/\.css(\?|$)/)) {
                        try { urls.add(new URL(r.name, base).href) } catch { }
                    }
                })
            }
        ...
        collectHtml() {
            ...
            document.querySelectorAll('a[href]').forEach(a => {
                try {
                    ...
                    if (u.origin === location.origin) {
                        const p = u.pathname
                        if (p.match(/\.html?(\?|$)/) || p.endsWith('/') || !p.includes('.')) urls.add(u.href)
                    }
                } catch { }
            })
            ...
            if (window.performance) {
                performance.getEntriesByType('resource').forEach(r => {
                    if (r.initiatorType === 'iframe' || r.name.match(/\.html?(\?|$)/)) {
                        try { urls.add(new URL(r.name, base).href) } catch { }
                    }
                })
            }
